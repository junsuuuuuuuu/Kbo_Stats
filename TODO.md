...
## 123456 - 내일 진행할 코드 최적화

- [ ] 중복 타석 파서 정리: `_plate_appearance_totals_v2`, `_v3` 제거 및 `_v4` 단일화
- [ ] 실시간 API 집계와 DB 박스스코어 집계의 공통 계산 로직 통합
- [ ] DB 저장 경기와 누락된 경기를 구분해 누락분만 API fallback 처리
- [ ] AVG·OBP·SLG·OPS별 데이터 상태를 독립적으로 판정하도록 `source_complete` 구조 개선
- [ ] 박스스코어 선수 `player_id` 연결 및 동명이인 검증
- [ ] 상세 API 실패 시 `failed` 스냅샷과 오류 메시지 저장
- [ ] 중복 파서 제거 후 전체 테스트·Ruff·최근 10경기 API 회귀 검증
