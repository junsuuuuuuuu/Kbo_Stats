...
### `GET /api/v1/players/{player_id}/overview`

선수 상세 화면의 기본 프로필과 전체 시즌 기록을 `player`, `seasons` 필드로 한 번에
반환한다. 브라우저의 중복 요청과 선수 존재 확인용 DB 재조회를 줄이기 위한 기본 상세
엔드포인트다. 기존 기본 정보·시즌 기록 엔드포인트도 호환성을 위해 유지한다.
...
수집된 최신 기준일을 구단별로 선택해 구단명, 등록 인원, 투수·야수 인원을 반환한다.

### `GET /api/v1/teams/{team_code}/roster?season=2026`

KBO 팀 코드로 최신 1군 등록 선수 명단을 반환한다. 선수 항목에는 KBO `player_id`, 등번호,
포지션, 투타, 생년월일, 신장·체중과 원본 URL이 포함된다. 존재하지 않으면
`404 TEAM_ROSTER_NOT_FOUND`를 반환한다.
...
### `GET /api/v1/teams/games/latest?season=2026`

### `GET /api/v1/teams/games/day?season=2026&game_date=2026-07-22`

수집 작업이 `game_day_snapshots`에 저장한 최신 경기일 또는 지정 날짜의 경기 일정·결과를
반환한다. API 요청 중 KBO 사이트를 호출하지 않으며 저장된 날짜가 없으면
`404 GAME_DAY_NOT_FOUND`를 반환한다.
